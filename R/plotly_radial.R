...
plotly_radial <- function(r_object, TEST) {
  ...
  if (length(r_object) == 13) {
    ...
    ay <- list(
      ...
      showline = FALSE,
      mirror = "ticks",
      gridcolor = plotly::toRGB("white"),
      gridwidth = 0,
      zerolinecolor = plotly::toRGB("black"),
      zerolinewidth = 1,
      linecolor = plotly::toRGB("white"),
      linewidth = 0,
      title = "Beta*sqrt(Wj)"
    )
    ...
    if (nrow(r_object$data[r_object$data$Outliers == "Outlier", ]) == 0) {
      T_PLOT <- plotly::plot_ly(
        ...
        plotly::add_annotations(
          ...
          text = c(
            round(r_object$confint[1], digits = 3),
            round(r_object$coef[1], digits = 3),
            round(r_object$confint[2], digits = 3)
          ),
          xref = "x",
          yref = "y",
          showarrow = TRUE,
  ...
  if (length(r_object) == 6) {
    if (TEST == TRUE) {
      ...
      circledat_EggerEST <- circle.radial(
        ...
        R.Egger,
        100,
        atan(Egger_circlemin),
        atan(Egger_circlemax)
      )
      ...
      if (nrow(r_object$data[r_object$data$Outliers == "Outlier", ]) == 0) {
        T_PLOT <- plotly::plot_ly(
          ...
          plotly::add_annotations(
            x = c(
              cxEgger[1],
              cos(atan(r_object$coef[2, 1])) * R.Egger,
              cxEgger[100]
            ),
            y = c(
              cyEgger[1],
              (sin(atan(r_object$coef[2, 1])) * R.Egger) + r_object$coef[1, 1],
              cyEgger[100]
            ),
            text = c(
              round(r_object$confint[1], digits = 3),
              round(r_object$coef[2, 1], digits = 3),
              round(r_object$confint[2], digits = 3)
            ),
            xref = "x",
            yref = "y",
