...
#' \item{\code{data}}{A data frame containing SNP IDs, inverse variance weights, the product of the inverse variance weight and ratio estimate for each variant, contribution to overall heterogeneity with corresponding p-value, and a factor indicator showing outlier status.}
...
egger_radial <- function(r_input, alpha, weights, summary) {
  ...
  if (
    ...
    stop(
      'The class of the data object must be "rmr_format", please resave the object with the output of format_radial().'
    )
  }
  ...
  if (weights == 3) {
    W <- ((r_input[, 5]^2 + (Egger.Slope^2 * r_input[, 4]^2)) /
      r_input[, 2]^2)^-1
  ...
  if (summary == TRUE) {
    ...
    cat(
      paste(
        c("\nF-statistic:", " on", " and"),
        round(EstimatesEGGER$fstatistic, 2),
        collapse = ""
      ),
      "DF, p-value:",
      format.pval(
        stats::pf(
          EstimatesEGGER$fstatistic[1L],
          EstimatesEGGER$fstatistic[2L],
          EstimatesEGGER$fstatistic[3L],
          lower.tail = FALSE
        ),
        digits = 3
      )
