...
plot_radial <- function(r_object, radial_scale, show_outliers, scale_match) {
  ...
  if (length(r_object) >= 6 && length(r_object) <= 13) {
    if (inherits(r_object, "IVW")) {
      ...
      Scalemin <- min(
        r_object$data[, 3] / r_object$data[, 2],
        r_object$confint[1]
      )
      ...
      circledat_ALLEST <- circle.radial(
        c(0, 0),
        R.All,
        length(r_object$data[, 3]),
        atan(Scalemin),
        atan(Scalemax)
      )
      ...
      circledat_IVWEST <- circle.radial(
        c(0, 0),
        R.IVW,
        length(r_object$data[, 3]),
        atan(r_object$confint[1]),
        atan(r_object$confint[2])
      )
      ...
      if (show_outliers == TRUE) {
        ...
        if (radial_scale == TRUE) {
          ...
          if (scale_match == TRUE) {
            ...
            if (
              nrow(r_object$data[r_object$data$Outliers == "Outlier", ]) == 0
            ) {
              #Produce plot showing full scale and all variants
              B <- ggplot2::ggplot(
                r_object$data,
                ggplot2::aes(x = Wj, y = BetaWj)
              ) +
                ggplot2::labs(title = "IVW Radial") +
                ggplot2::geom_point(ggplot2::aes(colour = "Variant")) +
                ggplot2::geom_path(ggplot2::aes(x = cxAll, y = cyAll)) +
                ...
                ggplot2::geom_text(
                  x = cos(atan(r_object$coef[1])) *
                    (R.IVW + Label.Scaling * 1.5),
                  y = sin(atan(r_object$coef[1])) *
                    (R.IVW + Label.Scaling * 1.5),
                  label = round(r_object$coef[1], digits = 3),
                  size = 4
                ) +
                ggplot2::geom_text(
                  x = cos(atan(r_object$confint[1]) - 0.02) *
                    (R.IVW + Label.Scaling),
                  y = sin(atan(r_object$confint[1]) - 0.02) *
                    (R.IVW + Label.Scaling) -
                    c(stats::quantile(r_object$data$Wj, seq(0, 0.1, 0.1))[1]),
                  label = round(r_object$confint[1], digits = 3),
                  size = 3
                ) +
                ggplot2::geom_text(
                  x = cos(atan(r_object$confint[2]) + 0.02) *
                    (R.IVW + Label.Scaling),
                  y = sin(atan(r_object$confint[2]) + 0.02) *
                    (R.IVW + Label.Scaling) +
                    c(stats::quantile(r_object$data$Wj, seq(0, 0.1, 0.1))[1]),
                  label = round(r_object$confint[2], digits = 3),
                  size = 3
                ) +
                ggplot2::theme_bw() +
                ggplot2::theme(
                ...
                ggplot2::annotate(
                  "segment",
                  x = 0,
                  xend = cos(atan(r_object$coef[1])) * R.IVW,
                  y = 0,
                  yend = sin(atan(r_object$coef[1])) * R.IVW,
                  colour = "black"
                ) +
                ...
                ggplot2::scale_y_continuous(limits = c(Y_MINIVW, Y_MAXIVW)) +
                ggplot2::coord_fixed() +
                ggplot2::scale_color_manual(
                  name = "Estimate",
                  breaks = c("Variant", "IVW"),
                  values = c("IVW" = "#56B4E9", "Variant" = "#000000")
                )
              ...
              if (max(abs(r_object$data[, 3] / r_object$data[, 2])) > 2) {
                ...
                for (i in seq_along(Y_Scale)) {
                  B <- B +
                    ggplot2::geom_text(
                      x = (cos(atan(Y_Scale[i])) * (R.All + Label.Scaling)),
                      y = (sin(atan(Y_Scale[i])) * (R.All + Label.Scaling)),
                      label = Y_Scale[i],
                      size = 2.5
                    )
                }
            ...
            if (
              ...
              maximal <- atan(
                max(abs(
                  r_object$data[r_object$data$Outliers == "Outlier", ]$BetaWj
                )) /
                  max(abs(
                    r_object$data[r_object$data$Outliers == "Outlier", ]$Wj
                  ))
              )
              R.All <- max(abs(
                r_object$data[r_object$data$Outliers == "Outlier", ]$BetaWj
              ...
              B <- ggplot2::ggplot(
                ...
                ggplot2::labs(title = "IVW Radial") +
                ggplot2::geom_point(ggplot2::aes(colour = Outliers)) +
                ggplot2::geom_path(ggplot2::aes(x = cxAll, y = cyAll)) +
                ggplot2::geom_path(ggplot2::aes(
                  x = cxIVW,
                  y = cyIVW,
                ...
                ggplot2::scale_colour_manual(
                  breaks = c("Outlier", "IVW"),
                  values = c("#56B4E9", "#E69F00", "white")
                )
              ...
              Theta <- rep(
                0,
                length(
                  r_object$data[r_object$data$Outliers == "Outlier", ]$Outliers
                )
              )
                ...
                B <- B +
                  ggplot2::geom_text(
                    ...
                    y = (r_object$data[r_object$data$Outliers == "Outlier", ][,
                      ...
                      0.9),
                    label = round(
                      r_object$data[r_object$data$Outliers == "Outlier", ][, 4][
                        i
                      ],
                      digits = 2
                    ),
          ...
          if (scale_match == TRUE) {
            ...
            if (
              ...
              B <- ggplot2::ggplot(
                ...
                ggplot2::scale_x_continuous(
                  limits = c(0, R.IVW + (Label.Scaling * 5)),
                  expand = c(0, 0)
                ) +
                ggplot2::scale_y_continuous(limits = c(Y_MINIVW, Y_MAXIVW)) +
                ggplot2::coord_fixed() +
    ...
    if (inherits(r_object, "egger")) {
      ...
      circledat_EGGEREST <- circle.radial(
        c(0, r_object$coef[1, 1]),
        R.Egger,
        length(r_object$data[, 3]),
        atan(r_object$confint[1]),
        atan(r_object$confint[2])
      ...
      Y_MINEGGER <- min(c(
        min(r_object$data[, 3]),
        (sin(atan(r_object$confint[1])) * (R.Egger + Label.Scaling_Egg)) +
          r_object$coef[1, 1],
        r_object$coef[1, 1] - Label.Scaling_Egg
      ))
      Y_MAXEGGER <- max(c(
        max(r_object$data[, 3]),
        (sin(atan(r_object$confint[2])) * (R.Egger + Label.Scaling_Egg)) +
          r_object$coef[1, 1],
        r_object$coef[1, 1] + Label.Scaling_Egg
      ))
      ...
      if (show_outliers == TRUE) {
        ...
        if (nrow(r_object$data[r_object$data$Outliers == "Outlier", ]) == 0) {
          ...
          B <- ggplot2::ggplot(
            ...
            ggplot2::labs(title = "MR-Egger Radial") +
            ggplot2::geom_point(ggplot2::aes(colour = "variant")) +
            ggplot2::geom_path(ggplot2::aes(
              x = cxEgger,
              y = cyEgger,
            ...
            ggplot2::geom_text(
              x = cos(atan(r_object$coef[2, 1])) *
                (R.Egger + Label.Scaling_Egg),
              y = sin(atan(r_object$coef[2, 1])) *
                (R.Egger + Label.Scaling_Egg) +
                r_object$coef[1, 1],
              label = round(r_object$coef[2, 1], digits = 3),
              size = 4
            ) +
            ggplot2::geom_text(
              x = cos(atan(r_object$confint[1])) *
                (R.Egger + Label.Scaling_Egg),
              y = (sin(atan(r_object$confint[1])) *
                (R.Egger + Label.Scaling_Egg)) +
                r_object$coef[1, 1],
              label = round(r_object$confint[1], digits = 3),
              size = 3
            ...
            ggplot2::geom_text(
              x = cos(atan(r_object$confint[2])) *
                (R.Egger + Label.Scaling_Egg),
              y = (sin(atan(r_object$confint[2])) *
                (R.Egger + Label.Scaling_Egg)) +
                r_object$coef[1, 1],
              label = round(r_object$confint[2], digits = 3),
              size = 3
            ...
            ggplot2::annotate(
              "segment",
              x = 0,
              xend = cos(atan(r_object$coef[2, 1])) * R.Egger,
              y = r_object$coef[1, 1],
              yend = (sin(atan(r_object$coef[2, 1])) * R.Egger) +
                r_object$coef[1, 1],
              colour = "black"
            ) +
            ggplot2::scale_x_continuous(
              limits = c(0, max(cxEgger) + Label.Scaling_Egg * 2),
              expand = c(0, 0)
            ) +
            ggplot2::scale_y_continuous(limits = c(Y_MINEGGER, Y_MAXEGGER)) +
            ggplot2::scale_color_manual(
              breaks = c("variant", "MR-Egger"),
              values = c("MR-Egger" = "#D55E00", "variant" = "#000000")
            ) +
            ggplot2::theme(legend.title = ggplot2::element_blank())
        ...
        if (nrow(r_object$data[r_object$data$Outliers == "Outlier", ]) > 0) {
          maximal <- atan(
            max(abs(r_object$data[r_object$data$Outliers == "Outlier", ][,
              3
            ])) /
              max(abs(r_object$data[r_object$data$Outliers == "Outlier", ][,
                2
          ...
          circledat_EGGEREST <- circle.radial(
            c(0, r_object$coef[1, 1]),
            R.Egger,
            length(r_object$data[r_object$data$Outliers == "Outlier", ][, 3]),
            atan(r_object$confint[1]),
            atan(r_object$confint[2])
          )
          cxEgger <- circledat_EGGEREST$x
          ...
          B <- ggplot2::ggplot(
            r_object$data[r_object$data$Outliers == "Outlier", ],
            ggplot2::aes(x = Wj, y = BetaWj)
          ) +
            ggplot2::labs(title = "MR-Egger Radial") +
            ggplot2::geom_point(ggplot2::aes(colour = Outliers)) +
            ...
            ggplot2::scale_y_continuous(limits = c(Y_MINEGGER, Y_MAXEGGER)) +
            ggplot2::scale_colour_manual(
              breaks = c("Variant", "Outlier", "MR-Egger"),
              values = c("#D55E00", "#E69F00", "#000000")
            ) +
            ggplot2::theme(legend.title = ggplot2::element_blank())
          ...
          for (i in seq_along(r_object$data[
            ...
            B <- B +
              ggplot2::annotate(
                "segment",
                x = r_object$data[r_object$data$Outliers == "Outlier", ][, 2][
                  i
                ],
                xend = r_object$data[r_object$data$Outliers == "Outlier", ][,
                  2
                ][i],
                y = r_object$data[r_object$data$Outliers == "Outlier", ][, 3][
                  i
                ],
                yend = r_object$data[r_object$data$Outliers == "Outlier", ][,
                  2
                ][i] *
                  r_object$coef[2, 1] +
                  r_object$coef[1, 1],
            ...
            B <- B +
              ggplot2::geom_text(
                ...
                y = (r_object$data[r_object$data$Outliers == "Outlier", ][, 3][
                  ...
                  0.9),
                label = round(
                  r_object$data[r_object$data$Outliers == "Outlier", ][, 4][i],
                  digits = 2
                ),
                size = 2.5
              )
  ...
  if (length(r_object) == 19) {
    ...
    R.All <- max(abs(r_object$data[, 3] - r_object$egger.coef[1, 1])) /
      sin(maximal) +
      stats::median(r_object$data[, 2])
    R.IVW <- R.All + (R.All / 12)
    ...
    Scalemin <- min(
      r_object$data[, 3] / r_object$data[, 2],
      r_object$IVW.confint[1],
      r_object$egger.confint[1]
    )

    Scalemax <- max(
      r_object$data[, 3] / r_object$data[, 2],
      r_object$IVW.confint[2],
      r_object$egger.confint[2]
    )
    ...
    circledat_EGGEREST <- circle.radial(
      c(0, r_object$egger.coef[1, 1]),
      R.Egger,
      length(r_object$data[, 3]),
      atan(r_object$egger.confint[1]),
      atan(r_object$egger.confint[2])
    )
    cxEgger <- circledat_EGGEREST$x
    cyEgger <- circledat_EGGEREST$y
    ...
    circledat_IVWEST <- circle.radial(
      ...
      R.IVW,
      length(r_object$data[, 3]),
      atan(r_object$IVW.confint[1]),
      atan(r_object$IVW.confint[2])
    )
    cxIVW <- circledat_IVWEST$x
    cyIVW <- circledat_IVWEST$y
    ...
    for (i in seq_along(r_object$data[, 3])) {
      if (
        r_object$data[, 6][i] == "Variant" &&
          r_object$egger.data[, 6][i] == "Variant"
      ) {
      ...
      if (
        r_object$data[, 6][i] == "Variant" &&
          r_object$egger.data[, 6][i] == "Outlier"
      ) {
        r_object$data$out_types[i] <- "MR-Egger Outlier"
      }

      if (
        r_object$data[, 6][i] == "Outlier" &&
          r_object$egger.data[, 6][i] == "Variant"
      ) {
    ...
    if (show_outliers == TRUE) {
      ...
      if (radial_scale == TRUE) {
        ...
        if (scale_match == TRUE) {
          #If no outliers are present
          if (
            nrow(r_object$data[r_object$data$out_types == "Variant", ]) ==
              length(r_object$data[, 3])
          ) {
            ...
            B <- ggplot2::ggplot(
              ...
              ggplot2::labs(title = "Radial Estimates") +
              ggplot2::geom_point(ggplot2::aes(colour = "Variant")) +
              ggplot2::geom_path(ggplot2::aes(x = cxAll, y = cyAll)) +
              ggplot2::geom_path(ggplot2::aes(
                x = cxIVW,
              ...
              ggplot2::geom_text(
                x = cos(atan(r_object$IVW.coef[1])) *
                  (R.IVW + Label.Scaling * 1.5),
                y = sin(atan(r_object$IVW.coef[1])) *
                  (R.IVW + Label.Scaling * 1.5),
                label = round(r_object$IVW.coef[1], digits = 3),
                size = 4
              ) +
              ggplot2::geom_text(
                x = cos(atan(r_object$IVW.confint[1]) - 0.02) *
                  (R.IVW + Label.Scaling),
                y = sin(atan(r_object$IVW.confint[1]) - 0.02) *
                  (R.IVW + Label.Scaling),
                label = round(r_object$IVW.confint[1], digits = 3),
                size = 3
              ) +
              ggplot2::geom_text(
                x = cos(atan(r_object$IVW.confint[2]) + 0.02) *
                  (R.IVW + Label.Scaling),
                y = sin(atan(r_object$IVW.confint[2]) + 0.02) *
                  (R.IVW + Label.Scaling),
                label = round(r_object$IVW.confint[2], digits = 3),
                size = 3
              ) +
              ggplot2::theme_bw() +
              ggplot2::theme(
              ...
              ggplot2::annotate(
                "segment",
                x = 0,
                xend = cos(atan(r_object$IVW.coef[1])) * R.IVW,
                y = 0,
                yend = sin(atan(r_object$IVW.coef[1])) * R.IVW,
                colour = "IVW"
              ) +
              ggplot2::scale_x_continuous(
                limits = c(0, max(cxEgger) + Label.Scaling_Egg * 2),
              ...
              ggplot2::geom_text(
                x = cos(atan(r_object$egger.coef[2, 1])) *
                  (R.Egger + Label.Scaling * 1.5),
                y = sin(atan(r_object$egger.coef[2, 1])) *
                  (R.Egger + Label.Scaling * 1.5) +
                  r_object$egger.coef[1, 1],
                label = round(r_object$egger.coef[2, 1], digits = 3),
                size = 4
              ) +
              ggplot2::geom_text(
                x = cos(atan(r_object$egger.confint[1])) *
                  (R.Egger + Label.Scaling * 1.5),
                y = (sin(atan(r_object$egger.confint[1])) *
                  (R.Egger + Label.Scaling * 1.5)) +
                  r_object$egger.coef[1, 1],
                label = round(r_object$egger.confint[1], digits = 3),
                size = 3
              ) +
              ggplot2::geom_text(
                x = cos(atan(r_object$egger.confint[2])) *
                  (R.Egger + Label.Scaling * 1.5),
                y = (sin(atan(r_object$egger.confint[2])) *
                  (R.Egger + Label.Scaling * 1.5)) +
                  r_object$egger.coef[1, 1],
                label = round(r_object$egger.confint[2], digits = 3),
                size = 3
              ) +
              ggplot2::annotate(
                "segment",
                x = 0,
                xend = cos(atan(r_object$egger.coef[2, 1])) * R.Egger,
                y = r_object$egger.coef[1, 1],
                yend = (sin(atan(r_object$egger.coef[2, 1])) * R.Egger) +
                  r_object$egger.coef[1, 1],
                colour = "black"
              ) +
              ggplot2::scale_color_manual(
                breaks = c("Variant", "IVW", "MR-Egger"),
                values = c(
                  "IVW" = "#56B4E9",
                  "MR-Egger" = "#D55E00",
                  "Variant" = "#000000"
          ...
          if (
            ...
            B <- ggplot2::ggplot(temp.dat, ggplot2::aes(x = Wj, y = BetaWj)) +
              ggplot2::labs(title = "Radial Estimates") +
              ggplot2::geom_point(ggplot2::aes(colour = out_types)) +
              ggplot2::geom_path(ggplot2::aes(x = cxAll, y = cyAll)) +
              ggplot2::geom_path(ggplot2::aes(
                x = cxIVW,
                y = cyIVW,
              ...
              ggplot2::scale_color_manual(
                ...
                values = c(
                  ...
                  "MR-Egger" = "#D55E00",
                  "Variant" = "white",
                  "IVW and MR-Egger Outlier" = "#CC79A7",
                  "MR-Egger Outlier" = "#E69F00",
                  "IVW Outlier" = "#009E73"
                )
              ) +
              ggplot2::theme(legend.title = ggplot2::element_blank())
      ...
      if (radial_scale == FALSE) {
        ...
        if (
          ...
          MINCILIM <- min(
            sin(atan(r_object$IVW.confint[1])) * R.IVW + Label.Scaling,
            sin(atan(r_object$egger.confint[1] - 1)) *
              (R.Egger + Label.Scaling_Egg - r_object$egger.coef[1, 1]),
            r_object$egger.coef[1, 1]
          )

          MAXCILIM <- max(
            sin(atan(r_object$IVW.confint[2])) * R.IVW + Label.Scaling,
            sin(atan(r_object$egger.confint[2] + 1)) *
              (R.Egger + Label.Scaling_Egg - r_object$egger.coef[1, 1]),
            r_object$egger.coef[1, 1]
          )
          ...
          B <- ggplot2::ggplot(
            ...
            ggplot2::scale_y_continuous(limits = c(Y_MIN, Y_MAX)) +
            ggplot2::scale_color_manual(
              breaks = c("Variant", "Outlier", "IVW", "MR-Egger"),
              values = c(
                "IVW" = "#56B4E9",
                "MR-Egger" = "#D55E00",
                "Variant" = "#000000",
        ...
        if (
          ...
          for (i in seq_along(temp.dat[, 3])) {
            if (
              abs(
                (temp.dat[, 3][i] - temp.dat[, 2][i] * r_object$IVW.coef[1])
              ) >
                abs(
                  (temp.dat[, 3][i] -
                    (temp.dat[, 2][i] *
                      r_object$egger.coef[2, 1] +
                      r_object$egger.coef[1, 1]))
                )
            ) {
              B <- B +
                ggplot2::annotate(
                  ...
                  xend = temp.dat[, 2][i],
                  y = temp.dat[, 3][i],
                  yend = temp.dat[, 2][i] * r_object$IVW.coef[1],
                  linetype = "solid",
                  colour = "#56B4E9"
                )
