...
plot_radial<-function(r_object,radial_scale,show_outliers,scale_match){
  ...
  if(length(r_object)==6){
    ...
    if(class(r_object)=="egger"){
      ...
      Y_MINEGGER<-min(  c(min(r_object$data[,3]),(sin(atan(r_object$confint[1]))*(R.Egger+Label.Scaling_Egg))+r_object$coef[1,1]  ))
      Y_MAXEGGER<-max(  c(max(r_object$data[,3]),(sin(atan(r_object$confint[2]))*(R.Egger+Label.Scaling_Egg))+r_object$coef[1,1]  ))
      ...
      if(show_outliers==TRUE){
        ...
        if(nrow(r_object$data[r_object$data$Outliers == "Outlier", ])==0){
          ...
          B<-ggplot(r_object$data,aes(x=Wj,y=BetaWj)
            ...
            scale_x_continuous(limits = c(0,cos(atan(r_object$coef[2,1]))*(R.Egger+(Label.Scaling_Egg*3))),expand=c(0,0))+scale_y_continuous(limits = c(Y_MINEGGER,Y_MAXEGGER))+scale_color_manual(breaks=c("variant","MR-Egger"),values=c("MR-Egger"="#D55E00","variant"="#000000"))+
        ...
        if(nrow(r_object$data[r_object$data$Outliers == "Outlier", ])>0){
          ...
          maximal<-atan(max(abs(r_object$data[r_object$data$Outliers == "Outlier", ][,3]))/max(r_object$data[r_object$data$Outliers == "Outlier", ][,2]))
          R.All<-max(abs(r_object$data[r_object$data$Outliers == "Outlier", ][,3]))/sin(maximal)
          R.Egger<-R.All+(R.All/12)
        ...
        if(nrow(r_object$data[r_object$data$Outliers == "Outlier", ])>0){
          ...
          B<-ggplot(r_object$data,aes(x=Wj,y=BetaWj)
            ...
            theme(panel.border = element_blank(),panel.grid.major = element_blank(),
                  panel.grid.minor = element_blank(),axis.line = element_line(colour = "black"))+ylab(expression(hat(beta)[j]~sqrt(W[j])))+xlab(expression(sqrt(W[j])))+
            geom_segment(aes(x = 0, xend = cos(atan(r_object$coef[2,1]))*R.Egger, y = r_object$coef[1,1], yend = (sin(atan(r_object$coef[2,1]))*R.Egger)+r_object$coef[1,1],colour="MR-Egger"))+
            scale_x_continuous(limits = c(0,cos(atan(r_object$coef[2,1]))*(R.Egger+(Label.Scaling_Egg*3))),expand=c(0,0))+scale_y_continuous(limits = c(Y_MINEGGER,Y_MAXEGGER))+scale_colour_manual(breaks=c("Variant","Outlier","MR-Egger"),values=c("#D55E00","#E69F00","#000000"))+
            theme(legend.title=element_blank())
    ...
    if(show_outliers==TRUE){
        ...
        if(scale_match==TRUE){
          ...
          if(nrow(r_object$data[r_object$data$out_types == "Variant", ])==length(r_object$data[,3])){
            ...
            B<-ggplot(r_object$data,aes(x=Wj,y=BetaWj)
              ...
              scale_x_continuous(limits = c(0,cos(atan(r_object$egger.coef[2,1]))*(R.Egger+(Label.Scaling*5))),expand=c(0,0))+scale_y_continuous(limits = c(Y_MIN,Y_MAX))+coord_fixed()+
          ...
          if(nrow(r_object$data[r_object$data$out_types == "Variant", ])<length(r_object$data[,3])){
            ...
            R.All<-max(abs(temp.dat[,3]))/sin(maximal)
            R.IVW<-R.All+(R.All/12)
            R.Egger<-R.All+(R.All/4)
          ...
          if(nrow(r_object$data[r_object$data$out_types == "Variant", ])==length(r_object$data[,3])){
            ...
            B<-ggplot(r_object$data,aes(x=Wj,y=BetaWj)
              ...
              theme_bw() +theme(panel.border = element_blank(),panel.grid.major = element_blank(),
                                ...
                                axis.line = element_line(colour = "black"))+ylab(expression(hat(beta)[j]~sqrt(W[j])))+xlab(expression(sqrt(W[j])))+
              geom_segment(aes(x = 0, xend = cos(atan(r_object$IVW.coef[1]))*R.IVW, y = 0, yend = sin(atan(r_object$IVW.coef[1]))*R.IVW,colour="IVW"))+
              scale_x_continuous(limits = c(0,cos(atan(r_object$egger.coef[2,1]))*(R.Egger+(Label.Scaling*5))),expand=c(0,0))+scale_y_continuous(limits = c(Y_MIN,Y_MAX))+
              geom_path(aes(x=cxEgger,y=cyEgger,colour="MR-Egger"))+
              geom_text(x=cos(atan(r_object$egger.coef[2,1]))*(R.Egger+Label.Scaling*1.5), y=sin(atan(r_object$egger.coef[2,1]))*(R.Egger+Label.Scaling*1.5) +r_object$egger.coef[1,1],label=round(r_object$egger.coef[2,1],digits=3),size=4)+
              geom_text(x=cos(atan(r_object$egger.confint[1]))*(R.Egger+Label.Scaling*1.5),y=(sin(atan(r_object$egger.confint[1]))*(R.Egger+Label.Scaling*1.5))+r_object$egger.coef[1,1],label=round(r_object$egger.confint[1],digits=3),size=3)+
              geom_text(x=cos(atan(r_object$egger.confint[2]))*(R.Egger+Label.Scaling*1.5),y=(sin(atan(r_object$egger.confint[2]))*(R.Egger+Label.Scaling*1.5))+r_object$egger.coef[1,1],label=round(r_object$egger.confint[2],digits=3),size=3)+
      ...
      if(radial_scale==FALSE){
        ...
        if(nrow(r_object$data[r_object$data$out_types == "Variant", ])==length(r_object$data[,3])){
          ...
          MINCILIM<-min(sin(atan(r_object$IVW.confint[1]))*R.IVW + Label.Scaling,sin(atan(r_object$egger.confint[1]))*(R.Egger +Label.Scaling_Egg-r_object$egger.coef[1,1]))
          
          MAXCILIM<-max(sin(atan(r_object$IVW.confint[2]))*R.IVW + Label.Scaling,sin(atan(r_object$egger.confint[2]))*(R.Egger +Label.Scaling_Egg-r_object$egger.coef[1,1]))
          ...
          B<-ggplot(r_object$data,aes(x=Wj,y=BetaWj)
            ...
            theme_bw() +theme(panel.border = element_blank(),panel.grid.major = element_blank(),
                              ...
                              axis.line = element_line(colour = "black"))+ylab(expression(hat(beta)[j]~sqrt(W[j])))+xlab(expression(sqrt(W[j])))+
            geom_segment(aes(x = 0, xend = cos(atan(r_object$IVW.coef[1]))*R.IVW, y = 0, yend = sin(atan(r_object$IVW.coef[1]))*R.IVW,colour="IVW"))+
            scale_x_continuous(limits = c(0,cos(atan(r_object$egger.coef[2,1]))*(R.Egger+(Label.Scaling*3))),expand=c(0,0))+geom_segment(aes(x = 0, xend = cos(atan(r_object$egger.coef[2,1]))*R.Egger, y = r_object$egger.coef[1,1], yend = (sin(atan(r_object$egger.coef[2,1]))*R.Egger)+
        ...
        if(nrow(r_object$data[r_object$data$out_types == "Variant", ])<length(r_object$data[,3])){
          ...
          B<-ggplot(r_object$data,aes(x=Wj,y=BetaWj)
            ...
            theme(panel.border = element_blank(),panel.grid.major = element_blank(),
                  panel.grid.minor = element_blank(),
                  axis.line = element_line(colour = "black"))+ylab(expression(hat(beta)[j]~sqrt(W[j])))+xlab(expression(sqrt(W[j])))+geom_segment(aes(x = 0, xend = cos(atan(r_object$IVW.coef[1]))*R.IVW, y = 0, yend = sin(atan(r_object$IVW.coef[1]))*R.IVW,colour="IVW"))+
            scale_x_continuous(limits = c(0,cos(atan(r_object$egger.coef[2,1]))*(R.Egger+(Label.Scaling*3))),expand=c(0,0))+geom_segment(aes(x = 0, xend = cos(atan(r_object$egger.coef[2,1]))*R.Egger, y = r_object$egger.coef[1,1], yend = (sin(atan(r_object$egger.coef[2,1]))*R.Egger)+r_object$egger.coef[1,1],colour="MR-Egger"))+
