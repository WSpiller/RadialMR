...
plot_radial<-function(r_object,radial_scale,show_outliers,scale_match){
  ...
  if(length(r_object)==12){
    ...
    R.All<-max(abs(r_object$data[,3]- r_object$egger.coef[1,1]))/sin(maximal)
    R.IVW<-R.All+(R.All/12)
    ...
    if(show_outliers==TRUE){
      ...
      if(radial_scale==FALSE){
          ...
          if(nrow(r_object$data[r_object$data$out_types == "Variant", ])==length(r_object$data[,3])){
            ...
            MINCILIM<-min(sin(atan(r_object$IVW.confint[1]))*R.IVW + Label.Scaling,sin(atan(r_object$egger.confint[1]))*(R.Egger +Label.Scaling_Egg-r_object$egger.coef[1,1]),r_object$egger.coef[1,1])
            
            MAXCILIM<-max(sin(atan(r_object$IVW.confint[2]))*R.IVW + Label.Scaling,sin(atan(r_object$egger.confint[2]))*(R.Egger +Label.Scaling_Egg-r_object$egger.coef[1,1]),r_object$egger.coef[1,1])
